...
function handleFileSelected(e) {
    ...
    if (isImage(singleFile.type)) {
        ...
        imgImageHolder.onload = function () {
            ...
            imgPreview.alt = 'Önizlemesi ' + singleFile.name;
            $('#imgPreview').remove();
...
function convert() {
    ...
    if (isImage) {
        ...
        setStatus('İkili veriler hazırlanıyor');
        byteArray = prepareBinary(binBinaryFileHolder);
    }
    setStatus('Verileri dizeye dönüştürme');
...
function prepareImage(image, newWidth, newHeight) {
    ...
    console.log('Görüntü boyutu: ' + image.width + 'x' + image.height + ' => ' + newWidth + 'x' + newHeight);
    ...
    console.log('süre: ' + ((new Date()) - dtStart) + 'ms');
    console.log('moddedPixels Boyutu: ' + moddedPixels.byteLength + 'bytes')
    console.log('yeni Pixels Boyutu: ' + newPixels.byteLength + 'bytes')
...
function convertToString(data, colNum, isImage, imageHeight, imageWidth) {
    console.log('Verileri dizeye dönüştürme');
    var resultString = '';
    ...
    resultString += '// Toplam Array Boyutu ' + dataLength + '\r\n';
...
function assebleSignature() {
    ...
    var sig = '';
	if ($('#cbcsharp').is(':checked')) {
		
		
     sig +='var '+ variableName + ' = new '+ $('#cbcsharp').val() ;
    }else{
...
function kapat() {
    ...
    $('#cbStatic').attr('disabled', true);
   $('#cbConst').attr('disabled', true);
	 $('#cbPROGMEM').attr('disabled', true);
}else{
	$('#cbStatic').attr('disabled', false);
   $('#cbConst').attr('disabled', false);
	 $('#cbPROGMEM').attr('disabled', false);
}
}
...
$('document').ready(function () {
    ...
    $.get('https://api.github.com/repos/lazenes/FileToCArray', function(data) {
